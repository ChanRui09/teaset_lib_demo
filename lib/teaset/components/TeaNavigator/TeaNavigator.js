...
function DynamicScreen({route, navigation}) {
  const {getRouteByKey, updateRouteRef, getDefaultRouteKey} = useContext(RouteRegistryContext);
  const routeKey = route?.params?.routeKey;
  const resolvedRouteKey = routeKey ?? getDefaultRouteKey();

  useEffect(() => {
    if (!routeKey && resolvedRouteKey) {
      navigation.setParams({routeKey: resolvedRouteKey});
    }
  }, [navigation, routeKey, resolvedRouteKey]);
  ...
  const element = useMemo(() => {
    ...
    return React.cloneElement(teaRoute.view, {ref: refCallback});
  }, [teaRoute, refCallback]);
        ...
        <View style={{flex: 1, backgroundColor: 'black'}}>
          <RouteRegistryContext.Provider value={registryValue}>
            ...
            >
              ...
              >
                <Stack.Screen name={SCREEN_NAME} component={DynamicScreen} />
              </Stack.Navigator>
            </NavigationContainer>
