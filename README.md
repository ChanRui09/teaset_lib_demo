...
- [x] 0.72 的 demo 中 Overlay 页面的 Popover 下的 shadow 属性异常(0.72 和 0.77 都不支持)(不在readme中，已删除)
- [ ] **0.72 在切换页面时状态栏闪烁**
- [ ] demo缺失部分枚举值，有些属性不生效
- 待更新
## 0.77 (基于0.72)
...
- [x] [**NavigationBar 的 statusbar 控制逻辑异常(设置隐藏 statusBar 之后，操作 hidden、 custom background 时，状态栏又会显示)**](https://github.com/ChanRui09/teaset_lib_demo/commit/881f8061fb98ec090d63c3e4a41d0095d49788bb)
- [x] 存在切换主题后状态栏不同步更新，进入任意界面后触发更新的问题
- [x] 0.77 的 demo 中 Overlay 页面的 Popover 下的 shadow 属性异常(0.72 和 0.77 都不支持)(不在readme中，已删除)
- [x] 0.77 存在些许闪屏问题, **AlbumView 界面点击图片缩略图，闪屏尤为明显**
- [ ] 横竖屏不生效，通过修改依赖库为ohos改善
- [ ] 还存在部分闪屏问题
- [ ] demo缺失部分枚举值，有些属性不生效
- 待更新
